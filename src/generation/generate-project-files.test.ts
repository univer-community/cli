...
describe("generation pipeline", () => {
  it("renders files from templates and omits monorepo-only conventions", () => {
    ...
    expect(getFile(files, "package.json")).toContain('"description": "A Univer Sheets plugin for Smart Filter."');
    expect(getFile(files, "package.json")).toContain('"homepage": "https://github.com/univer-community"');
    expect(getFile(files, "package.json")).toContain('"license": "MIT"');
    expect(getFile(files, "package.json")).toContain('"author": "Anonymous"');
    expect(getFile(files, "package.json")).toContain('"rxjs": "^7.8.2"');
    expect(getFile(files, "package.json")).toContain('"oxfmt": "^0.44.0"');
    expect(getFile(files, "package.json")).toContain('"oxlint": "^1.59.0"');
    expect(getFile(files, "package.json")).toContain('"simple-git-hooks": "^2.13.1"');
    expect(getFile(files, "package.json")).toContain('"@univerjs/design": "^0.19.0"');
    expect(getFile(files, "package.json")).toContain('"fmt": "oxfmt"');
    expect(getFile(files, "package.json")).toContain('"fmt:check": "oxfmt --check"');
    expect(getFile(files, "package.json")).toContain('"lint": "oxlint"');
    expect(getFile(files, "package.json")).toContain('"prepare": "simple-git-hooks"');
    expect(getFile(files, "package.json")).toContain('"pre-commit": "pnpm lint:fix && pnpm fmt"');
    expect(getFile(files, ".editorconfig")).toContain("indent_size = 2");
    expect(getFile(files, ".npmignore")).toContain("!LICENSE");
    expect(getFile(files, "LICENSE")).toContain("Copyright (c) 2026 univer-community");
    expect(getFile(files, "oxfmt.config.ts")).toContain("sortPackageJson: true");
    expect(getFile(files, "oxlint.config.ts")).toContain('plugins: ["unicorn", "typescript", "oxc", "import"]');
    expect(getFile(files, "oxlint.config.ts")).not.toContain("ignorePatterns");
    expect(getFile(files, "README.md")).toContain("## Installation");
    expect(getFile(files, "README.md")).toContain("## Local Development");
    expect(getFile(files, "README.md")).toContain("## API Notes");
    expect(getFile(files, "README.md")).toContain("## Build Output");
    expect(getFile(files, "README.md")).toContain("## Demo");
    expect(getFile(files, "README.md")).toContain("## License");
    expect(getFile(files, "README.md")).toContain("### Register the plugin");
    expect(getFile(files, "README.md")).toContain("### Facade Entry");
    expect(getFile(files, "package.json")).toContain('"./lib/*": "./lib/*"');
    expect(getFile(files, "package.json")).toContain('"./lib/facade"');
    expect(getFile(files, "scripts/build.mjs")).toContain('import { build as tsdownBuild } from "tsdown";');
    expect(getFile(files, "scripts/build.mjs")).toContain('const OUTPUT_DIRECTORY = "lib";');
    expect(getFile(files, "scripts/build.mjs")).toContain('entryFileNames: "[name].js"');
