...
import { DEFAULT_PACKAGE_NAME, DEFAULT_PROJECT_VERSION, DEFAULT_UNIVER_VERSION, SHAPE_CHOICES, SURFACE_CHOICES } from '../constants';
...
function validateVersion(value: string): string | null {
  return /^\d+\.\d+\.\d+(-[\w.-]+)?$/.test(value)
    ? null
    : 'Please enter a semver-like version such as 0.19.0.';
}
...
export async function collectGenerationOptions(initialTargetDir?: string, promptSession?: PromptSession): Promise<GenerationOptions> {
  const rl = promptSession ?? createPromptSession();
  const ownsSession = !promptSession;

  try {
    const targetDir = await askInput(rl, 'Output directory', initialTargetDir ?? './univer-plugin-project', validateDirectory);
    const pluginSlug = ensureKebabCase(await askInput(
      rl,
