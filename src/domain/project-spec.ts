...
export function deriveProjectBlueprint(input: GenerationOptions): ProjectBlueprint {
  return {
    ...
    baseUiDependencyImport: input.shape === 'logic-ui' || input.shape === 'mobile-ui-addon'
      ? BASE_UI_DEPENDENCY[input.surface]
      : undefined,
  };
}
...
function deriveDependencies(input: GenerationOptions, blueprint: ProjectBlueprint): ProjectDependencies {
  ...
  const peer: Record<string, string> = blueprint.includeUiPlugin || blueprint.includeMobileEntry
    ? {
      react: '^18.0.0 || ^19.0.0',
...
function deriveScripts(blueprint: ProjectBlueprint): ProjectScripts {
  return {
    ...
    lintFix: 'oxlint --config .oxlintrc.json . --fix',
    test: 'vitest run',
